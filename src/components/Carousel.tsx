...
export function Carousel({ products }: CarouselProps) {
  ...
  const itemsPerPage = screenSize.isMobile ? 1 : screenSize.isTablet ? 3 : 4
      ...
      <div className="flex items-center justify-center max-w-full">
        ...
        <div className={`flex gap-1.5 lg:gap-2 ${screenSize.isMobile ? 'overflow-x-auto snap-x snap-mandatory justify-center' : 'justify-center'}`} style={{ maxWidth: '100%' }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          {getVisibleProducts().map((product, i) => (
            <div key={`${currentIndex}-${i}`} className="relative flex-shrink-0" style={{ width: screenSize.isMobile ? '70vw' : screenSize.isTablet ? 'calc((100% - 60px) / 3)' : 'calc((100% - 60px) / 4)', maxWidth: screenSize.isMobile ? '280px' : 'none' }} onMouseEnter={() => !screenSize.isMobile && setHoveredIndex(i)} onMouseLeave={() => !screenSize.isMobile && setHoveredIndex(null)}>
              <div className={screenSize.isMobile ? 'h-auto' : 'h-full'} style={{ minHeight: screenSize.isMobile ? 'auto' : '280px' }}>
                <div className="relative flex flex-col h-full overflow-hidden rounded-xl" style={{ backgroundColor: !screenSize.isMobile && hoveredIndex === i ? '#c8921e' : 'rgba(255,255,255,0.08)', border: '1px solid rgba(200,150,30,0.2)' }}>
                  ...
                  <div className="flex-1 flex items-center justify-center p-3 lg:p-4 relative" style={{ zIndex: 5 }}>
                    <img src={product.image} alt={product.name} className="w-full max-w-[100px] lg:max-w-[120px] object-contain" style={{ filter: 'drop-shadow(0 6px 12px rgba(0,0,0,0.2))', transform: !screenSize.isMobile && hoveredIndex === i ? 'scale(1.05)' : 'scale(1)', transition: 'transform 0.2s ease' }} />
                  </div>
                  ...
                  <div className={screenSize.isMobile ? 'relative' : 'absolute bottom-0 left-0 right-0'} style={{ backgroundColor: 'rgba(13,10,4,0.92)', display: screenSize.isMobile || hoveredIndex === i ? 'block' : 'none', zIndex: 20 }}>
                    <div className="p-3 lg:p-4">
                      ...
                      <button className="w-full py-2 text-xs lg:text-sm font-medium transition-all duration-200" style={{ backgroundColor: '#c8921e', color: '#0d0a04', fontFamily: 'Oswald, sans-serif' }}>Saber Mais</button>
                    </div>
                  </div>
                </div>
              </div>
